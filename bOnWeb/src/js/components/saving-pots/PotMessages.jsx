...
const PotMessages = React.createClass({
  propTypes: {
  ...
  conditionCheck(cYear, nYear, cMonth, nMonth, trgAmount, monthlyContribution, timeRequired, expMonth, expYear, expContribution, differentiator, expectedTimeSpan) {
    ...
    if (this.props.goal !== 0 && this.props.goal !== null && this.props.isChanged) {
      ...
      if (trgAmount <= 0 && this.props.goal > 0) {
        ...
        if (timeRequired >= 1200 || mc === 0) {
          let message = (<div><h5>{content.potMessageHead2}</h5>
                          <p>
                            {content.potMessage2Part1} <strong>{mcCurrency}</strong> {content.potMessage2Part5} <strong>{goalValue}</strong>,
                           {content.potMessage2Part3}<br/> {content.potMessage8Part3}
                          </p></div>);
          return (<div>{message}</div>);
        } else {
          if (mc === ec) {
            ...
            let message = (<div><h5>{content.potMessageHead8}</h5>
                            <p>
                              {content.potMessage2Part1} <strong>{mcCurrency}</strong> {content.potMessage7Part2} <strong>{finalChangedGoal}</strong> {content.potMessage5Part3} {exDate}.
                             <br/>{content.potMessage7Part3}
                            </p></div>);
            return (<div>{message}</div>);
          } else if (mc < ec) {
              let message = (<div><h5>{content.potMessage7Part4}</h5>
                              <p> {content.potMessage2Part1} <strong>{mcCurrency}</strong> {content.potMessage7Part5} <strong>{goalValue}</strong> {content.savingpotByBal} {nDate}.<br/>
              ...
              if (differentiator === 'ON_TARGET_BUT_OVER') {
                ...
                let message = (<div><h5>{content.savingPotwayToGoHead}</h5>
                                <p>
                                  {content.potMessage2Part1} <strong>{mcCurrency}</strong> {content.potMessage5Part2} <strong>{newAmountCurrency}</strong> {content.potMessage5Part3} {nDate}.<br/>
                                   {content.potMessage5Part4} <strong>{ecCurrency}</strong> {content.potMessage5Part6} {exDate}.
                                </p></div>);
                return (<div>{message}</div>);
              } else {
                let message = (<div><h5>{content.potMessage7Part6}</h5>
                                <p>
                               {content.potMessage2Part1} <strong>{mcCurrency}</strong> {content.potMessage7Part8} <strong>{goalValue}</strong> {content.savingpotByBal} {nDate}.<br/>
                                {content.potMessage5Part4} <strong>{ecCurrency}</strong> {content.potMessage8Part1} {exDate}.
                                </p></div>);
                return (<div>{message}</div>);
              }
            }
